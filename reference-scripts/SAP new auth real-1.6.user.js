// ==UserScript==
// @name         SAP new auth real
// @namespace    http://tampermonkey.net/
// @version      1.6
// @description  Suivi des commandes en attente de numéro SAP avec compliance Kingfisher API v3.0 (KITS headers & WAF bypass).
// @author       You
...
(function() {
    ...
    // -----------------------------
    // AUTH CAPTURE (COMPUTER-AGNOSTIC)
    // -----------------------------
    ...
    (function hookNativeXHR() {
        ...
        XMLHttpRequest.prototype.setRequestHeader = function(h, v) {
            ...
            this._sap_headers[h.toLowerCase()] = v; // Normalize keys to lowercase for easier lookup later
                ...
                this.addEventListener('load', () => {
                    if (this.status >= 200 && this.status < 300) {
                        ...
                        if (KD_HOST_RE.test(url)) {
                            // Specifically look for critical KITS headers to save
                            ...
                            if(snap.headers['kits-workstation-id']) {
                                console.log('[SAP] ✅ Captured Critical KD Headers (Workstation ID found).');
                            }
                        }
                    }
                }, { once: true });
    ...
    // Helper to clean forbidden headers for standard requests
    function stripAndBuild(baseHeaders, defaults, extra = {}) {
        ...
        const out = {};
        // baseHeaders keys expected to be lowercase from capture
        for (const [k, v] of Object.entries(baseHeaders || {})) {
            if (!forbidden.has(k.toLowerCase())) out[k] = v;
        }
        Object.entries(defaults || {}).forEach(([k,v]) => { out[k] = v; });
        Object.assign(out, extra);
        return out;
    }

    // Prod-Agent Request (Legacy/ATG)
    ...
    // Kingfisher API Request (Digital Colleague v3.0 Compliance)
    function kdRequest({ method, url, data = null, headers = {}, timeout = 15000 }) {
        // 1. Retrieve captured headers to find dynamic values (Workstation ID, Store Code)
        const captured = kdAuth.headers || {};

        // 2. Build KITS Headers (Guide v3.0 Section 2.B)
        // Note: Captured headers keys are stored in lowercase in hookNativeXHR
        const kitsHeaders = {
            'kits-operating-company': 'CF01', // Guide: Must be CF01, not CAFR
            'kits-tenant-id': 'CAFR',
            'kits-application-name': 'DigitalColleague',
            'kits-app-version': '2.0.0',
            'kits-device-id': `desktop_${Date.now()}`, // Guide: Dynamic timestamp
            'kits-workstation-id': captured['kits-workstation-id'] || '', // MANDATORY
            'kits-store-code': captured['kits-store-code'] || ''          // MANDATORY
        };

        // 3. Build WAF Bypass Headers (Guide v3.0 Section 2.A)
        const wafHeaders = {
        ...
        // 4. Merge
        const finalHeaders = Object.assign({}, wafHeaders, kitsHeaders, headers);

        // Alert if critical headers are missing
        if (!finalHeaders['kits-workstation-id']) {
            console.warn('[SAP] ⚠️ Missing kits-workstation-id. Authentication may fail. Please browse DC to capture headers.');
        }

        return new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                method, url, data, headers: finalHeaders,
                cookie: kdAuth.cookie || undefined, // Session cookie is critical
                timeout,
                onload: resolve,
                onerror: reject,
                ontimeout: () => reject(new Error('Timeout')),
    ...
    async function validateKfplcAuth() {
        ...
        if (!hasKdHeaders()) {
            console.warn('[SAP] No KD headers yet; Validate request will likely fail (missing workstation-id).');
        }
        try {
            // kdRequest now handles the specific KITS and WAF headers required by the guide
            const r = await kdRequest({ method: 'GET', url });
    ...
    function init() {
        ...
        console.log("Suivi SAP Avancé (v1.6) initialisé avec support API Kingfisher v3.0");
