// Casto Tools — module « Provisionnés » (v1.4.0)
// Port 1:1 du userscript Tampermonkey éponyme : import d'un CSV des
// provisionnés (mapping par EAN), injection du taux de provision, du PV
// Plancher et des provisionnés restants sur les pages produit Com+
// (dc.kfplc.com), plus un filtre « Provisionnés » dans les résultats de
// recherche.
//
// Différences avec le userscript (voir README) :
//   - GM_get/setValue → chrome.storage.local (core/storage.js) — l'API du
//     script était déjà async, le port est direct (mêmes clés de stockage)
//   - GM_addStyle     → styles.css injecté par le manifest
//   - boot()          → CastoTools.register('provisionnes', init)
'use strict';

(() => {

  const LOG_PREFIX = '[Casto Tools · Prov]';
  ...
  async function getLastImportInfo() {
    try {
      const info = await castoStorage.get(LAST_IMPORT_KEY, null);
      return info;
    } catch { return null; }
  }
  ...
  async function setLastImportInfo(count) {
    ...
    await castoStorage.set(LAST_IMPORT_KEY, info);
    return info;
  }
  ...
  function showToast(message, type = 'success') {
    ...
    toast.innerHTML = `${SVG_ICONS.check}<span></span>`;
  ...
  function ensureModal() {
    ...
    load.addEventListener('click', async () => {
      ...
      try {
        ...
        await castoStorage.set(STORAGE_KEY, data);
        DATA = data;
  ...
  // ---------- Data ----------
  let DATA = {};
  async function loadData() { DATA = await castoStorage.get(STORAGE_KEY, {}); if (!DATA || typeof DATA !== 'object') DATA = {}; }
  ...
  async function loadFilterPref() {
    try { PROV_FILTER_ONLY = !!(await castoStorage.get(FILTER_STORAGE_KEY, false)); }
    catch { PROV_FILTER_ONLY = false; }
  }
  async function setFilterPref(v) {
    PROV_FILTER_ONLY = !!v;
    try { await castoStorage.set(FILTER_STORAGE_KEY, PROV_FILTER_ONLY); } catch {}
  }
  ...
  CastoTools.register('provisionnes', init);
